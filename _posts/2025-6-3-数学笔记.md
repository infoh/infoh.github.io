...
&ensp;&ensp;&ensp;&ensp;完备性公理内容为：如果 $S$ 是实数集合 $\mathbb{R}$ 的非空子集且有上界，则 $S$ 存在最小上界且是实数。If $\varnothing \ne S \subset \mathbb{R}$, then there exists  $a = l.u.b.\  S, \ a\in \mathbb{R}$。纯粹从概念上看，实数集合由于满足域公理和序公理而具有有序域的本质，但并非一定要是”域“才具有最小上界。一个有序集合，即使不满足域公理，也可能存在最小上界。这种特性被称为有序集合的最小上界性（the least-upper-bound property）。可以证明，最大下界性和最小上界性是等同的概念。最后，如果采取通过有理数来构造实数的办法来定义实数集合，那么完备性公理是可以被证明的，因此有时它被称为原理而不被视为公理。

&ensp;&ensp;&ensp;&ensp;符合以上公理的集合，换句话说，也就是具有最小上界性的有序域，被定义为实数集合，它的成员叫实数。

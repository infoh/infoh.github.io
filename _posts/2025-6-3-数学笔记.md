...
&ensp;&ensp;&ensp;&ensp;完备性公理内容为：如果 $S$ 是实数集合 $\mathbb{R}$ 的非空子集且有上界，则必有：$S$ 存在最小上界且是实数。If $\varnothing \ne S \subset \mathbb{R}$, then there exists  $a = l.u.b.\  S, \ a\in \mathbb{R}$。也就是说，集合 $\mathbb{R}$ 的任何非空子集，只要该子集有上界，那么一定能在 $\mathbb{R}$ 中找到该子集的最小上界。这一特性虽然不明确该子集是否连续，但它保证了 $\mathbb{R}$ 是连续的，没有“空洞”，因为一旦有空洞，那么以该空洞为右端点的任何开区间（即子集）就不存在最小上界。纯粹从语言逻辑上看，某类有序集合，由于它的禀性（比如连续性），即使不满足域公理，仍然可能使它的任何有上界的非空子集恒有最小上界，且该最小上界不一定属于对应的非空子集，但一定属于作为父集的有序集合。这种特性被称为有序集合的最小上界性（the least-upper-bound property）。（全体有理数构成的集合 $Q$ 就不具有最小上界性，因为平方值小于 $2$ 的所有正有理数构成的子集，它的最小上界不属于 $Q$）。可以证明，最大下界性和最小上界性是等同的概念。最后，采取恰当的方法可以利用有理数来构造一个有序域，使它包含有理数集作为子集，如果能够证明该有序域具有最小上界性，那么就是把完备性公理当作原理进行证明，同时证明了这样的有序域是存在的。因此有时该表述被称为原理而不被视为公理。

&ensp;&ensp;&ensp;&ensp;实数集合符合以上所有公理，这说明它本质上是具有最小上界性的有序域，并且包含了有理数集作为其子集。
